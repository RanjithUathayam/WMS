...
@Component({
  ...
  styleUrls: ['./inventory-details-report.component.css']
})
export class InventoryDetailsReportComponent implements OnInit {
  ...
  columns: ReportColumn[] = [
    ...
    { header: 'Row', data: 'rowCode' },
    { header: 'Location', data: 'locationCode', sortable: true },
    ...
    { header: 'Created At', data: 'createdAt', sortable: true }
  ];
  ...
  load(): void {
    ...
    this.apiservice.getReportData('inventorydetails', this.buildFilters(), this.page, this.pageSize, this.sortBy, this.sortDir).subscribe((res: any) => {
      this.loading = false;
